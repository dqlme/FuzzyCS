...
class Oort(SelectionBase):
    def __init__(self, args):
        self.alpha = args.alpha_oort
        self.pacer_step = args.pacer_step
        self.pacer_window = args.step_window

    def select(self, round_idx, client_num_in_total, client_num_per_round, metric, run_time, T):
        if client_num_in_total == client_num_per_round:
            ...
            utility_all = self.compute_utility(metric, metric, run_time, T)

            client_indexes = np.append(selected, selected2, axis=0)

        return client_indexes.astype(int)

    def compute_utility(self, metric, run_time, T):
        #  compute utility
        loss = metric
        stat_util = math.sqrt(loss)
        indicator = 1 if T < run_time else 0
        indicator *= self.alpha
        sys_util = (T / run_time) ** indicator
        utility = stat_util * sys_util
